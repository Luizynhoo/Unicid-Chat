...
export function WelcomeMessage() {
  return (
    <div className="welcome">
      ...
      <p className="welcome__subtitle">Estou aqui para ajudá-lo com:</p>
      <ul className="welcome__list">
        {welcomeCapabilities.map((item) => (
          <li key={item.label} className="welcome__list-item">
            <span className="welcome__emoji" aria-hidden="true">{item.emoji}</span>
            {item.label}
          </li>
        ))}
      </ul>
      <p className="welcome__cta">Como posso ajudá-lo hoje?</p>
    </div>
  );
}
