...
export default function MetricsGrid({ data }) {
  const totalFeedbacks = data.chat_ajudou + data.chat_nao_ajudou;

  const taxaSucesso = totalFeedbacks > 0
    ? ((data.chat_ajudou / totalFeedbacks) * 100).toFixed(1)
    : '0.0';

  const successNum = parseFloat(taxaSucesso);

  const successAccent =
    successNum >= 70 ? 'success' :
    successNum >= 40 ? 'warning' :
    'danger';
